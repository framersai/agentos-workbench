    ...
    <section className="flex h-full flex-col overflow-hidden rounded-xl border theme-border theme-bg-secondary-soft p-3 transition-colors duration-300">
      <header className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="rounded-full border border-slate-300 bg-white/70 px-2.5 py-0.5 text-xs text-slate-600 dark:border-slate-700/40 dark:bg-slate-900/80 dark:text-slate-300">
      ...
      <div className="mb-3 rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-xs text-slate-600 dark:border-white/10 dark:bg-slate-900/40 dark:text-slate-400">
      ...
      <div className="mb-3 space-y-2 flex-shrink-0">
        <label className="flex flex-col gap-1 text-[10px] uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">
          Search personas
          <input
        ...
        {capabilityOptions.length > 0 && (
          <div className="flex flex-wrap items-center gap-1 text-[10px]">
            ...
            {capabilityOptions.length > 8 && (
              <span className="text-slate-500">+{capabilityOptions.length - 8}</span>
            )}
            {filterActive && (
              <button
                type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto space-y-3">
        <ul className="grid grid-cols-1 gap-2 text-sm text-slate-700 lg:grid-cols-2 dark:text-slate-200">
          {visiblePersonas.map((persona) => (
            <li key={persona.id} className="flex flex-col rounded-xl border border-slate-200 bg-white p-2.5 dark:border-white/5 dark:bg-slate-950/50">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{persona.displayName}</p>
                  {persona.description && <p className="mt-0.5 text-xs text-slate-600 dark:text-slate-400 line-clamp-2">{persona.description}</p>}
                </div>
              ...
              {persona.tags && persona.tags.length > 0 && (
                <div className="mt-1.5 flex flex-wrap gap-1">
                  {persona.tags.slice(0, 5).map((tag) => (
                    <span key={tag} className="rounded bg-slate-100 px-1 py-0.5 text-[9px] font-medium uppercase tracking-wider text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                      {tag}
                    </span>
                  ))}
              ...
              {persona.capabilities && persona.capabilities.length > 0 && (
                <div className="mt-1 flex flex-wrap gap-1">
                  {persona.capabilities.slice(0, 4).map((cap) => (
                    <span key={cap} className="rounded bg-sky-100 px-1 py-0.5 text-[9px] font-medium uppercase tracking-wider text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">
        ...
        {personas.length > visiblePersonas.length && (
          <div className="rounded-xl border border-dashed border-slate-300 px-3 py-2 text-center text-xs text-slate-500 dark:border-white/10">
            {personas.length - visiblePersonas.length} more personas in store
          </div>
        )}

        <form onSubmit={handleSubmit} className="space-y-2 rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700 dark:border-white/10 dark:bg-slate-950/50 dark:text-slate-200">
          <div className="flex items-center justify-between">
            <p className="uppercase tracking-[0.35em] text-slate-500 dark:text-slate-500">Quick add (basic)</p>
            <Sparkles className="h-3 w-3 text-slate-400" />
          </div>
          <p className="text-[10px] text-slate-500">For full config (system prompt, guardrails, extensions), use the Wizard above.</p>
          <label className="space-y-0.5 block">
            <span className="text-slate-500 dark:text-slate-400">Display name</span>
            <input
              value={draft.displayName}
              onChange={(event) => setDraft((prev) => ({ ...prev, displayName: event.target.value }))}
              className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-900 focus:border-sky-500 focus:outline-none dark:border-white/10 dark:bg-slate-950/80 dark:text-slate-100"
              placeholder="Aurora QA Specialist"
            />
          </label>
          <label className="space-y-0.5 block">
            <span className="text-slate-500 dark:text-slate-400">Purpose / description</span>
            <textarea
